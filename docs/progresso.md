...
- [x] **Log textual de estado da visão** (mão detectada/perdida, gesto reconhecido, protocolo disparado) via `logger.info` em `visionService.js` — visível em `pm2 logs kevin`, criado especificamente porque a janela de debug (`--show`, `cv2.imshow`) não funciona num Pi headless sem X11.
- [x] **Firmware da luz: log serial + suporte a relé ativo-em-LOW.** Sintoma: `/luz on`/`off` "funcionava" (ESP32 respondia `ok`) mas o relé físico não reagia. Log de recebimento de comando e do nível de GPIO escrito adicionados; flag `RELAY_ACTIVE_LOW` (a maioria dos módulos de relé baratos aciona invertido) criada pra facilitar o diagnóstico via Serial Monitor. Confirmado funcionando depois do ajuste — usuário reportou "funcionou".
- [x] **Luz por linguagem natural**: `intentRouter.js` (antes só usado na voz) ganhou `light_on`/`light_off`, plugado em `index.js` — qualquer mensagem de texto sem comando explícito passa por `classifyIntent()` antes do chat normal. `/luz on`/`/luz off` continuam existindo como atalho sem custo extra. Testado com 8 casos (incluindo "tá escuro aqui" como armadilha de falso positivo) antes de commitar — todos corretos. Ver [decisoes.md](decisoes.md).

- [x] **Music Assistant rodando (no PC, não no Pi) + Spotify conectado.** Tentativa original era rodar no Pi (Docker) — bateu em falta de espaço no cartão SD de 14GB, depois em camada de imagem corrompida, e por fim em erro de containerd travado pós-instalação (`_ping` 500 error) mesmo tentando corrigir com storage externo — decidido migrar pro PC pra não continuar perdendo tempo em infra do Pi. Docker Desktop + WSL2 no Windows, host networking ativado (suportado desde Docker Desktop 4.34). Spotify: fluxo de autenticação padrão redirecionava pro IP interno da VM do Docker Desktop (`192.168.65.3`) em vez de `localhost`, corrigido configurando **Base URL** (`http://localhost:8095`) em Settings → Core do MA. **Correção de decisão**: Squeezelite ia ser instalado no PC — errado, o player tem que ser o Pi (é ele que está conectado na caixa de som do quarto). Passo a passo corrigido em [deploy.md](deploy.md) Fase 11 — ainda não executado.
...
- [ ] Testar a caixa de som Bluetooth (Fase 6 do deploy.md) — pré-requisito do Squeezelite abaixo, não mencionado ainda nas sessões de troubleshooting
- [ ] Instalar e testar Squeezelite no Pi como player do Music Assistant (deploy.md Fase 11) — depende do item acima (Bluetooth) estar funcionando primeiro
- [ ] Decidir como o Squeezelite sobrevive a reboot do Pi (app no `ecosystem.config.js` ou serviço systemd — não decidido ainda)
- [ ] Implementação do lado do Kevin ainda não começou: `music/engine.py` (sidecar), `music/interface/musicService.js`, classificador de intenção de música — tudo isso depende do player (Squeezelite) estar confirmado funcionando primeiro
