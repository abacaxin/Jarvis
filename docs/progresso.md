...
- [x] **Causa da Fase 4 travada encontrada, fix proposto — falta validar no Pi de verdade.** `vision/requirements.txt` fixava `mediapipe>=0.10,<0.11`, cap escolhido antes do Python 3.13 existir. Confirmado via PyPI: não existe NENHUM wheel 0.10.x pra Python 3.13, em nenhuma plataforma (nem build from source funciona, ver [google-ai-edge/mediapipe#6159](https://github.com/google-ai-edge/mediapipe/issues/6159)). A versão 1.0.0 (mais nova que o cap, por isso nunca considerada) resolve isso: mudou de wheel por versão do Python (cp310/cp311/cp312) pra wheel `py3-none` (independente de versão do CPython) — inclui `linux aarch64`, deve instalar em qualquer Python 3.x. Pin trocado pra `mediapipe>=1.0.0`. **Ainda não testado no Pi**: nem o `pip install` em si, nem se a API usada em `detection/hand_detector.py` (Tasks API, `HandLandmarker`) segue idêntica no salto 0.10→1.0 — rodar `pip install -r vision/requirements.txt` de novo e depois um teste de detecção de verdade (`python vision/test/test_hand_detection.py` ou similar) antes de considerar resolvido.
