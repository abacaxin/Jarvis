...
- [x] **Music Assistant rodando (no PC, não no Pi) + Spotify conectado.** Tentativa original era rodar no Pi (Docker) — bateu em falta de espaço no cartão SD de 14GB, depois em camada de imagem corrompida, e por fim em erro de containerd travado pós-instalação (`_ping` 500 error) mesmo tentando corrigir com storage externo — decidido migrar pro PC pra não continuar perdendo tempo em infra do Pi. Docker Desktop + WSL2 no Windows, host networking ativado (suportado desde Docker Desktop 4.34). Spotify: fluxo de autenticação padrão redirecionava pro IP interno da VM do Docker Desktop (`192.168.65.3`) em vez de `localhost`, corrigido configurando **Base URL** (`http://localhost:8095`) em Settings → Core do MA. Squeezelite (decisão já tomada, ver decisoes.md) ainda não instalado/confirmado como player do PC.
