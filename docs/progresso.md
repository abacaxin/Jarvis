...
- [x] **`/foto` testado no Pi (mediapipe já instalado via Python 3.11), sem reação — causa raiz encontrada e corrigida.** ESP32 confirmado conectando no hub (`dispositivo registrado: quarto_luz` nos logs), Kevin online — mas `pm2 logs kevin` mostrou `ModuleNotFoundError: No module named 'mediapipe'` ao ativar a visão. Causa: `VISION_PYTHON_BIN` continuava comentado em `ecosystem.config.js` (o próprio `deploy.md` já avisava disso na Fase 4, mas o código nunca tinha sido ajustado pra valer) — PM2 não herda o venv ativado, então `spawn('python3', ...)` pegava o Python 3.13 do sistema (sem mediapipe) em vez do venv com Python 3.11. Corrigido: `VISION_PYTHON_BIN: '/home/softh/kevin/venv/bin/python3'` descomentado e preenchido em `ecosystem.config.js`. **Falta**: `git pull` no Pi + recriar a app no PM2 (mudar `env` de uma app existente às vezes não pega só com `restart`, mais seguro `pm2 delete kevin && pm2 start ecosystem.config.js`) + retestar `/pare` → `/foto` → gesto de punho fechado.
