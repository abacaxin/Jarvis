...
**Decisão 2 — onde toca de verdade:** o modelo de player do MA é por protocolo específico (plugin Python dentro do próprio servidor MA) — não existe um jeito genérico de "qualquer processo Node se registrar como player". A solução é **Squeezelite** (cliente leve, suporte nativo maduro no MA) — o Kevin não precisa construir nada pra isso, é configuração de player, não código.

**Correção (2026-08-14):** a ideia original era rodar o Squeezelite no PC — errado. O player tem que ser o **Raspberry Pi**, que é o hub do quarto e está conectado na caixa de som — é ele quem deve emitir o som de verdade, não o PC. O Music Assistant (servidor) continua no PC só por causa da dor de cabeça de infra do Pi (ver bloco abaixo); o **player** (Squeezelite) roda no Pi, conecta na caixa Bluetooth já pareada (Fase 6 do deploy.md) e aponta pro MA no PC pela rede. `apt install squeezelite` existe direto no Raspberry Pi OS, não precisa compilar.
